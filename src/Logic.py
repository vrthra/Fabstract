...
def can_generalize(tval, dtree, grammar, predicate, unverified, max_checks, node):
    return are_these_abstract(dtree, grammar, predicate, [tval] + unverified, max_checks, node[0])

def are_these_abstract(dtree, grammar, predicate, abstract_paths, max_checks, msg):
    checks = 0
    limit = 0
    abstract = True
    rstr = None
...
def shallow_abstraction(tval, dtree, grammar, predicate, max_checks):
    path, status = tval
    node = get_child(dtree, path)
    ...
    abstract = are_these_abstract(dtree, grammar, predicate, [tval], max_checks, node[0])
    if abstract:
        return [(path, None)]
    else:
        if is_token(key): return []
        ...
        for i,child in enumerate(children):
            if not is_nt(child[0]): continue
            p = shallow_abstraction((path + [i], None), dtree, grammar, predicate, max_checks)
            paths.extend(p)
        return paths
